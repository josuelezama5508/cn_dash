...
class MailHistoryController extends API
{
    private $userModel;
    private $services = [];
    ...
    public function get($params = [])
    {
        try {
            [$action, $search] = $this->resolveAction($params, [
                'getMailIdPago' => 'getMailIdPago',
                'getMailNog' => 'getMailNog',
                'getMailNogAction'=> 'getMailNogAction'
            ]);
            $service = $this->service('HistoryMailControllerService'); 

            $map = [
                'getMailIdPago' => fn() => $service->getByRowId($search),
                'getMailNog' => fn() => $service->getMailByNog($search, $this->service('BookingControllerService')),
                'getMailNogAction' => fn() => $service->getByRowIdAndAction(33, "transporte"),
            ];
            $response = $map[$action]();
            if (empty($response)) {
